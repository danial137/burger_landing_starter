...
const Hero = () => {
    return (
        <Carousel additionalTransfrom={0} arrows={false} autoPlay={true} autoPlaySpeed={4000} centerMode={false} infinite responsive={responsive} itemClass='item' showDots={true}>
            ...
            {/* 2st hero imgae  */}
            <div className='w-[100%] h-[88vh] flex items-center justify-center flex-col bg-red-900 md:clip_path'>
                <div className='w-[80%] grid grid-cols-1 md:grid-cols-2 justify-center items-center gap-[2rem] mx-auto'>
                    {/* imge */}
                    <Image src={BurgerImg2} alt='burger' className='hidden md:block' />
                    {/* information */}
                    <div>
                        <h1 className='text-[40px] font-semibold text-yellow-400'>Tasty Food Burger </h1>
                        <h1 className='text-[90px] leading-[5rem] font-bold uppercase text-white'>5 star <br /> Burgers</h1>
                        <p className='mt-[1rem] text-white text-opacity-70 text-[18px]'>
                        ...
                        <button className='px-8 mt-[2rem] py-3 text-[16px] bg-green-600 transition-all duration-200 hover:bg-blue-500 flex items-center rounded-md space-x-2 text-white'>
                            <span>
                                <BiCycling className='w-[1.7rem] h-[1.7rem] ' />
                            </span>
                            <span className='font-bold'>
